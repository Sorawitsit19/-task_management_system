...
# นำ Class Task จาก oop_tasks.py มาใช้
class Task:
            ...
            with open(self.filename, "r") as f:
                for line in f:
                    ...
                    if len(parts) >= 4:
                        ...
                        due_date = parts[2].strip() if parts[2].strip() != 'None' else None
...
class TaskManager:
    def __init__(self, storage: TaskStorage): # รับ Data Storage เข้ามา
    ...
    def add_task(self, description, due_date=None, priority="medium"):
        ...
        self.next_id += 1
        self.storage.save_tasks(self.tasks) # บันทึกหลังเพิ่ม
    ...
    def mark_task_completed(self, task_id):
        ...
        if task:
            task.mark_completed()
            self.storage.save_tasks(self.tasks) # บันทึกหลังเปลี่ยนสถานะ
            return True
